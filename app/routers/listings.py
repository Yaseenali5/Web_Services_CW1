...
def create_listing(listing: schemas.ListingCreate, db: Session = Depends(get_db), _: str = Depends(require_api_key)):
...
def update_listing(listing_id: int, listing: schemas.ListingUpdate, db: Session = Depends(get_db), _: str = Depends(require_api_key)):
    if not crud.get_region(db, listing.region_id):
        raise HTTPException(status_code=404, detail="Region not found")
...
def delete_listing(listing_id: int, db: Session = Depends(get_db), _: str = Depends(require_api_key)):
